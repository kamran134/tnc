    ...
    <footer className="snap-start bg-gray-900 text-white flex items-center" style={{ minHeight: '100vh' }}>
      <div className="container-max section-padding w-full">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Company Info */}
          <div>
            <h3 className="text-2xl font-bold mb-4">{companyInfo?.companyName || 'TnC'}</h3>
            ...
            <div className="flex space-x-4">
              ...
              {companyInfo?.facebookUrl && (
                <a href={companyInfo.facebookUrl} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              {companyInfo?.instagramUrl && (
                <a href={companyInfo.instagramUrl} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <div className="space-y-2 text-gray-300">
              ...
              {companyInfo?.phone && <p>{t('footer.phone')}: {companyInfo.phone}</p>}
              {companyInfo?.email && <p>{t('footer.email')}: {companyInfo.email}</p>}
              {companyInfo?.website && (
                <p>
                  <a href={companyInfo.website} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">
                    {companyInfo.website}
                  </a>
                </p>
              )}
